...
>
> ⚠️ **CRITICAL ARCHITECTURAL CONSTRAINT & WARNING** ⚠️
> **EVERY SLIDE IS A SINGLE GENERATED IMAGE**: In this system, we do NOT render any HTML, CSS, or SVG code on the slide canvas itself. Every slide is a single, full-bleed PNG image generated 100% by the text-to-image model (Imagen).
>
> Because of this, **this file is read and interpreted directly by the AI models (including Imagen)**. You must strictly adhere to the following prompt-writing constraints:
> 1. **NO CSS/CODE PROPERTIES**: Never use CSS properties like `border-radius: 24px`, `box-shadow`, or coding syntax in the style descriptions. Use natural language instead (e.g. `smooth rounded corners`, `subtle soft drop shadows`).
> 2. **NO PIXEL OR DIMENSIONAL MEASUREMENTS**: Imagen does not understand pixels. Never use measurements like `exactly 1.5 px to 2 px stroke weight` or `48x48 px`. Use qualitative descriptors instead (e.g. `thin outlines`, `fine geometric lines`, `hairline strokes`).
> 3. **NO HEX CODES IN IMAGE PROMPTS**: Text-to-image models do not understand hex color codes (like `#F8F9FA`). Including them in prompts can cause severe text hallucinations (rendering the hex string literally on the slide). Always use descriptive, natural color names in the prompts (e.g. `royal blue`, `navy blue`, `pure white`, `charcoal gray`).
>
> *Note: The Hex codes in the Color Palette table below are only for the text models (Gemini) in Stage 2/3 to maintain a conceptual understanding of the color scheme when planning layout structures. They must never be passed to the Imagen prompt.*
>
> **Why this file matters**: Since every slide image is generated independently by the image model in separate calls with no shared memory, this file is the *only* shared anchor that keeps the deck looking like one cohesive presentation. Vague or ambiguous descriptions here are the #1 cause of visual drift — be concrete and leave no room for the model to improvise.
...
> - **Professional Business (Light)**: Always use a structured, high-contrast light corporate palette: Background pure white `#FFFFFF` or light gray `#F8F9FA`, Surface light gray `#F1F3F4` for cards, Text Primary dark charcoal `#202124` or deep navy `#1C3D5A`, Text Secondary gray `#5F6368`. Use a single primary corporate color (e.g., royal blue `#1A73E8` or deep navy `#1A237E`) for accents and monochrome icons.
...
- **Content (QR Code) Slides**: Specifically designed for presenting a QR code (e.g. for feedback, contact, download, or links). Must use a structured asymmetric 60/40 or 65/35 vertical split layout. One side contains the structured content cards (e.g. two stacked cards with icons and text), while the opposite side features a dedicated, prominent container card with soft rounded corners and wobbly/soft drop shadows. The center of this container card must contain a **completely blank, solid light-gray or white square placeholder**. Under no circumstances should you draw any QR code patterns, pixels, or dots inside this placeholder; it must be a completely solid, empty square. The bottom of the card contains a centered caption label (e.g., "掃描前往 GitHub 專案庫") rendered in matching typography.
...
* **Aesthetic**: Clean, friendly, highly structured card-based layouts. Features generous white space, thin light-gray divider lines, and card containers with very smooth, large rounded corners and a very subtle soft drop shadow.
* **Icons**: Official **Material Symbols (Outlined)** style — clean, geometric, friendly outline icons with rounded line caps and joins, a consistent medium stroke weight, no fill.
...
### 🎨 Theme B: Professional Business Infographic — Best for Financial Reports, Business Operations, Data-Driven Pitches, Technical Architecture
* **Aesthetic**: Structured, data-dense, and highly polished corporate style in bright Light Mode. Specially designed for presenting numbers, metrics, KPIs, and complex processes clearly. Uses a crisp white or ultra-light cool gray background. Relies on clean grids, structured columns, and borderless card containers with extremely soft, flat drop shadows. Employs thin light-gray divider lines or elegant primary-colored top accent borders to organize dense information with mathematical precision. High-contrast, clean sans-serif typography with a strong emphasis on giant hero numbers and bold metric callouts.
* **Icons**: Monochrome, flat, ultra-clean business outline icons (rendered in a single solid dark corporate color like deep navy blue or slate gray, a consistent very thin stroke weight) placed cleanly without any background containers, cards, or 3D effects.
* **Illustrations & Imagery (Stylized Vector Infographics & Diagrams)**: Strictly forbid physical photography, 3D clay, wobbly sketches, or playful cartoon characters. Instead, use high-end **minimalist 2D or subtle isometric vector diagrams, flowcharts, data visualizations, and technical schematics**. Features clean geometric lines, solid color fills, and a strictly limited brand color palette, looking like a premium dashboard widget or a professional technical blueprint.
* **Semantic Metaphor Rule**: Every visual must be a precise, clean schematic representing the slide's technical or business concept (e.g., a clean multi-layer stack diagram for system architecture, a precise circular cycle diagram for a feedback loop, a clean bar/line chart graphic with a target line for growth metrics).
* **Imagen Keyphrase**: `Premium flat vector infographic, clean professional business diagram, minimalist 2D data visualization, geometric shapes, clean lines, solid color fills, professional corporate color palette, high-end tech corporate style, transparent background`

### 🎨 Theme C: 3D Claymorphism — Best for Modern Startups, Consumer Tech, Creative Agency, Friendly Corporate
...
* **Aesthetic**: Cozy, organic, sketching-paper texture (warm cream), human-centered, and inviting. Uses soft, wobbly lines and gentle pastel colors. **Never use pure black for text; use Deep Navy Slate for a gentle pencil/sketch feel.**
