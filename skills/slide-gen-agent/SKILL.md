...
* **Semantic Alignment Rule (語意對齊與真實隱喻原則)**: Instruct that every generated illustration or image must serve as a direct, meaningful representation of the slide's core message (e.g. a puffy 3D clay lock for security, a real photo of a key for access). **Strictly forbid meaningless abstract shapes or placeholders.**
* **Icon System & CSS Containers**: Define the icon style and size. For Theme B (3D Claymorphism), you must write the **Claymorphic Container** CSS rules (solid light background, soft outer drop shadow, and multiple inner highlights/shadows to create a puffy cushion effect) to seamlessly embed flat icons into 3D space. You must also write the **Icon Color Layout Rules** (Category Contrast, Sequence Progression, Focus & Accentuation) into the file, instructing downstream tools on how to distribute colors across slide elements professionally.
