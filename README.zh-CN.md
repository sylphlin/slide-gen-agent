...
3. **阶段 3：图片生成与预览**
   ...
   - 所有幻灯片图片与演讲备注将被编译成一个 `preview.html` 页面，便于轻松预览与审阅。
   - *流程会直接进入阶段 4。*
...
5. **阶段 5：演示文稿打包与下载**
   - 一旦您批准最终的幻灯片，代理会提供四种导出选项：
     - **Google 幻灯片**：代理会将 PPTX 上传至 Google 云端硬盘，在 `slide-gen-agent` 文件夹中转换为 Google 幻灯片文件，并与您分享编辑权限。可直接在浏览器中打开、编辑与分享。*(需要在 GCP 中启用 Google Drive API，并在 Google Workspace 管理员控制台中配置域范围委派。)*
     - **PPTX（含演讲备注的 PowerPoint）**：宽屏 PowerPoint 文件，包含幻灯片图片，且演讲稿已完全嵌入每张幻灯片的备注区域。文件名会使用演示文稿主题（例如 `ai-trends-2025.pptx`）。
...
* **使用场景**：支持 Agent Skill、提供沙盒代码执行环境、且具备文本生成图像能力的 LLM 系统（例如 Antigravity、Codex）。
* **如何安装**：
  1. 将 [SKILL.md](file:///Users/sylph/Documents/Antigravity/slide-gen-agent/skills/slide-gen-agent/SKILL.md) 的内容复制并导入您的 LLM 助理的自定义系统指令或系统提示词中。
...
#### Option 1：一键安装 (One-Click Installation)
