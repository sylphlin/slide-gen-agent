...
├── deploy.sh                # インタラクティブな自動デプロイスクリプト
├── deploy_details.md        # 詳細デプロイガイド
├── deploy/
│   └── terraform/           # GCP リソース構築用 Terraform 設定ファイル
│       ├── main.tf
│       ├── variables.tf
│       └── outputs.tf
├── skills/
...
└── adk_agent/               # プログラム実装のホストエージェント（Python ADK 2.0 実装）
    ├── requirements.txt     # Python の依存関係設定（python-pptx を含む）
    ├── agent.py             # エージェントのメインエントリーポイント
    ├── config.py            # 環境変数およびエージェント設定マネージャー
    └── tools/               # エージェントツール
        ├── __init__.py
