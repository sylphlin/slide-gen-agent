...
Before deploying, you must set your Google Cloud Project ID and Project Number as environment variables:
```bash
export GOOGLE_CLOUD_PROJECT="your-actual-gcp-project-id"
...
Once environment variables are configured, dependencies are installed, and the virtual environment is active, run the ADK deployer from the `adk_agent` directory:
```bash
adk deploy agent_engine \
  ...
  .
```
*Behind the scenes, the ADK CLI handles containerization, deployment staging, and Reasoning Engine registration. When the command completes, it will output your **Reasoning Engine Resource ID** (e.g., `projects/${GOOGLE_CLOUD_PROJECT_NUMBER}/locations/us-central1/reasoningEngines/{ENGINE_ID}`).*

#### 4. Configure IAM Permissions

##### A. Build & Deploy Permissions (One-Time Setup)
If the deployment command fails with a "Build failed" error, your project's default compute service account (`${GOOGLE_CLOUD_PROJECT_NUMBER}-compute@developer.gserviceaccount.com`) might lack permission to write build logs or push built images.
...
3. **Grant permissions to the agent's runtime service account**:
   - Locate your project's runtime identity (typically the **Compute Engine default service account**: `${GOOGLE_CLOUD_PROJECT_NUMBER}-compute@developer.gserviceaccount.com`).
...
#### 5. Connect to Gemini Enterprise Console
...
#### 6. (Optional) Enable Google Slides Export
